...
/// The three colors used to paint the dot grid. Bundling them in a value type
/// lets each context (the app, a Home Screen widget, a Lock Screen accessory)
...
struct DotPalette {
    ...
    /// Monochrome palette for Lock Screen accessories, which the system renders
    /// with vibrancy. Here opacity, not hue, conveys progress.
...
/// Draws one dot per day of the year, sizing them to fill the available space.
///
...
struct DotGrid: View {
    ...
    var body: some View {
        Canvas { context, size in
            let layout = GridLayout(count: progress.totalDays, in: size)
            let radius = layout.cell * dotScale / 2
            guard radius > 0 else { return }
            let today = progress.dayOfYear
        ...
        .accessibilityValue(
            "Day \(progress.dayOfYear) of \(progress.totalDays), \(progress.percent) percent elapsed"
        )
    }
}
...
private struct GridLayout {
    ...
    init(count: Int, in size: CGSize) {
        ...
        // `count` is at most 366, so this loop is trivial and runs once per draw.
        var bestColumns = 1
